...
function liffToggleDeviceLedState(state) {
 ...
 if(state==2){
    window.ledCharacteristic.writeValue(new Uint8Array([0x02]) 
    ).catch(error => {
        uiStatusError(makeErrorMsg(error), false);
    });
  }
 if(state==3){
    window.ledCharacteristic.writeValue(new Uint8Array([0x03])
    ).catch(error => {
        uiStatusError(makeErrorMsg(error), false);
    });
  }
 if(state==4){
    window.ledCharacteristic.writeValue(new Uint8Array([0x04]) 
    ).catch(error => {
        uiStatusError(makeErrorMsg(error), false);
    });
  }
